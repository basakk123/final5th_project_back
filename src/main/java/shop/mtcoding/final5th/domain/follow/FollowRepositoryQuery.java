...
public class FollowRepositoryQuery {
        ...
        public FollowCountRespDto findFollowCountByFollowingUserId(Long followingUserId) {
                StringBuffer sb = new StringBuffer();
                sb.append(
                                "select count(*) as followCount from follow fo where fo.following_user_id = :followingUserId");
        ...
        public FollowerCountRespDto findFollowerCountByUserId(Long userId) {
                StringBuffer sb = new StringBuffer();
                sb.append(
                                "select count(*) as followerCount from follow fo where fo.user_id = :userId");

                Query query = em.createNativeQuery(sb.toString())
                                .setParameter("userId", userId);

                JpaResultMapper result = new JpaResultMapper();
                FollowerCountRespDto followerCountRespDto = result.uniqueResult(query, FollowerCountRespDto.class);
                return followerCountRespDto;
        }
}
